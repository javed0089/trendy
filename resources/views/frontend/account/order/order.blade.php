  ...
  <div class="container">
    ...
    <div class="row about-sidebar">
      ...
      <div class="col-md-10 about-content">
        ...
        @endif
        <div class="panel-div">
          <div class="panel-title">{{__('Order No.')}}: {{$order->id}}</div>
          <div class="content">
            <div class="row">

              <div class="col-md-4">
                <h4>{{__('Order No.')}} : <span >{{$order->order_no}}</span></h4>
                <h4>{{__('Status')}} : <span>{{$order->Status->status_en}}</span></h4>

              </div>
              <div class="col-md-4">
                <h4>{{__('Dated')}} : <span>{{date('M j, Y',strtotime($order->created_at))}}</span></h4>
        ...
        <div class="col-md-4 pull-right text-right">
          ...
          <h3>{{__('Grand Total')}}: {{number_format($total,2)}}</h3>
        </div>
      </div>
      ...
      <div class="panel-div">
        <div class="panel-title">{{__('Order Documents')}}
          <span class="pull-right small">{{count($order->OrderFiles)}} {{__('document(s)')}}</span>
        </div>
        <div class="content">
          <div class="row">
            ...
            <div class="col-md-12">
              <form class="form-inline pull-right" role="form"  method="Post" enctype="multipart/form-data" action="{{route('myorders.update',$order->id)}}">
                {{csrf_field()}}
                {{ method_field('PATCH') }}
                <label class="mr-sm-2" for="inlineFormCustomSelect">{{__('Document Type')}}</label>
    ...
    <div class="panel-div">
      <div class="panel-title">{{__('Track your order Shipment')}}
              ...
              <div class="col-md-12">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                      ...
                      <h4>{{__('Tracking ID')}}: {{$order->shipping_tracking_id}}</h4>
                      <h4>{{__('Tracking link')}}:  <a href="{{url($order->shipping_tracking_hyperlink)}}" target="_blank">{{__('Click here')}}</a>  </h4>
...
<div class="comments1">
  <h3 class="title-2 text-center"> {{count($order->OrderComments->where('is_private','==','0'))}} {{__('Messages')}}</h3>
  ...
  <form action="{{route('myorders.update',$order->id)}}" method="POST"  class="commentform single-click-form" data-parsley-validate>
    ...
    <div class='row'>
      ...
      <div class="comment-btn col-md-12">
        <button type="submit" name="submit" value="addComment" class="btn btn-block btn-warning"> {{__('Send Message')}} </button>
      </div>
    </div>
  </form>
